...
4. **`test_residual_connection`**：了解殘差連接的重要性
5. **`test_attention_and_ffn_contribute`**：理解各組件的貢獻
6. **`test_forward_with_cache`**：初步了解緩存機制

### 第三階段：進階特性

7. **`test_past_key_value_reuse`**：深入理解增量式生成和緩存重用
8. **`test_moe_vs_standard_ffn`**：了解 MoE 架構的特點和優勢
