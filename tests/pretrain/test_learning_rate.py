...
from train_pretrain import get_lr
...
class TestLearningRate:
    ...
    def test_lr_schedule_visualization(self):
        ...
        plt.title('學習率調度曲線', fontproperties=FontProperties(family=self.font))
    ...
    @pytest.mark.parametrize("base_lr", [1e-3, 5e-4, 1e-4])
    ...
    def test_compare_different_schedules(self):
        ...
        plt.title('不同學習率調度策略比較', fontproperties=FontProperties(family=self.font))
        plt.xlabel('訓練步數', fontproperties=FontProperties(family=self.font))
        plt.ylabel('學習率', fontproperties=FontProperties(family=self.font))
