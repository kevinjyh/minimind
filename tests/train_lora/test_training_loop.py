...
import train_lora
from train_lora import train_epoch, get_lr
...
class TestLoRATrainingLoop:
    ...
    def setup_training_dependencies(self):
        ...
        # 模擬 LoRA 參數設置
        def mock_parameters():
            # 返回一個參數列表，包含正常參數和 LoRA 參數
            normal_param = torch.nn.Parameter(torch.ones(1), requires_grad=False)
            lora_param = torch.nn.Parameter(torch.ones(1), requires_grad=True)
            return [normal_param, lora_param]
        
        mock_model.parameters = mock_parameters
        
        # 模擬 named_parameters 方法以識別 LoRA 參數
        def mock_named_parameters():
            normal_param = torch.nn.Parameter(torch.ones(1), requires_grad=False)
            lora_param = torch.nn.Parameter(torch.ones(1), requires_grad=True)
            return [('layer.0.weight', normal_param), ('layer.0.lora.A.weight', lora_param)]
        
        mock_model.named_parameters = mock_named_parameters
        ...
        def mock_backward(*args, **kwargs):
            ...
            if accumulation_step_counter % mock_args.accumulation_steps == 0:
                ...
                # 確保梯度裁剪被調用，但只對 LoRA 參數 (lora_params)
                torch.nn.utils.clip_grad_norm_(lora_params, mock_args.grad_clip)
        ...
        mock_args.lora_name = "test_lora"
        os.makedirs(os.path.join(mock_args.save_dir, "lora"), exist_ok=True)
        ...
        # 模擬 LoRA 參數
        lora_params = [torch.nn.Parameter(torch.ones(1), requires_grad=True)]
        ...
        def wrapped_train_epoch(epoch, wandb):
            ...
            try:
                ...
                # 使用新的全局環境調用原始函數
                old_globals = dict(train_lora.__dict__)
                train_lora.__dict__.update(globs)
                
                # 調用訓練函數
                train_epoch(epoch, wandb)
                
                # 恢復原始全局環境
                train_lora.__dict__.clear()
                train_lora.__dict__.update(old_globals)
                
            finally:
                # 恢復原始函數
                torch.nn.CrossEntropyLoss = original_loss_fct
                torch.nn.utils.clip_grad_norm_ = original_clip_grad_norm
                train_lora.save_lora = original_save_lora
        ...
        return {
            ...
            "save_lora": mock_save_lora,
            "loss_fct": mock_loss_fct,
            "wrapped_train_epoch": wrapped_train_epoch,
            "lora_params": lora_params
        }
    
    def test_lr_update_in_lora_train_epoch(self, setup_training_dependencies):
        """測試 LoRA 訓練循環中的學習率更新"""
        # 獲取模擬對象和嵌套函數
        ...
        # 監控 get_lr 函數調用
        with patch('train_lora.get_lr', wraps=get_lr) as mock_get_lr:
            # 調用嵌套的訓練循環
        ...
        # 記錄調用參數
        with open(os.path.join(self.output_dir, "lora_lr_update_calls.txt"), "w", encoding="utf-8") as f:
            f.write(f"LoRA 訓練中的學習率被更新\n")
            f.write(f"最終學習率值: {mock_optimizer.param_groups[0]['lr']}\n")
    
    def test_lora_backward_and_optimization_steps(self, setup_training_dependencies):
        """測試 LoRA 訓練中的反向傳播和優化步驟"""
        # 獲取模擬對象和嵌套函數
        ...
        # 記錄調用順序和頻率
        with open(os.path.join(self.output_dir, "lora_optimization_steps.txt"), "w", encoding="utf-8") as f:
    ...
    def test_lora_gradient_clipping(self, setup_training_dependencies):
        """測試 LoRA 訓練中的梯度裁剪（僅對 LoRA 參數）"""
        # 獲取模擬對象和嵌套函數
        ...
        try:
            ...
            # 檢查裁剪參數是否為 lora_params
            args_list = mock_clip_grad_norm.call_args_list
            for args, kwargs in args_list:
                # 第一個參數應該是 lora_params，第二個參數應該是 grad_clip
                assert args[0] == setup_training_dependencies["lora_params"], "梯度裁剪應只對 LoRA 參數進行"
        ...
        # 記錄調用信息
        with open(os.path.join(self.output_dir, "lora_gradient_clipping.txt"), "w", encoding="utf-8") as f:
            f.write(f"LoRA 梯度裁剪被調用了 {mock_clip_grad_norm.call_count} 次\n")
            f.write(f"裁剪參數類型: {type(setup_training_dependencies['lora_params']).__name__}\n")
            f.write(f"裁剪閾值: {setup_training_dependencies['args'].grad_clip}\n")
    
    def test_lora_save_during_training(self, setup_training_dependencies):
        """測試 LoRA 訓練期間的權重保存 (使用 save_lora 而非 torch.save)"""
        # 獲取模擬對象和嵌套函數
        mock_save_lora = setup_training_dependencies["save_lora"]
        wrapped_train_epoch = setup_training_dependencies["wrapped_train_epoch"]
        ...
        # 檢查 save_lora 是否被調用
        assert mock_save_lora.called, "save_lora 應當被調用"
        
        # 記錄保存調用
        with open(os.path.join(self.output_dir, "lora_save_calls.txt"), "w", encoding="utf-8") as f:
            f.write(f"save_lora 被調用了 {mock_save_lora.call_count} 次\n")
            
            # 記錄保存路徑
            args_list = mock_save_lora.call_args_list
            for i, (args, kwargs) in enumerate(args_list):
                f.write(f"調用 {i+1} - 模型: {args[0]}, 保存路徑: {args[1]}\n")
    
    def test_lora_loss_calculation(self, setup_training_dependencies):
        """測試 LoRA 訓練中的損失計算"""
        # 獲取模擬對象和嵌套函數
        ...
        # 記錄調用信息
        with open(os.path.join(self.output_dir, "lora_loss_calculation.txt"), "w", encoding="utf-8") as f:
            f.write(f"LoRA 訓練中的損失計算正確執行\n")
    
    def test_lora_parameter_updates(self, setup_training_dependencies):
        """測試 LoRA 參數更新（確保只有 LoRA 參數在更新）"""
        # 創建一個跟蹤 LoRA 參數更新的模擬模型
        mock_model = MagicMock()
        
        # 模擬 named_parameters 方法，返回一些正常參數和一些 LoRA 參數
        def mock_named_parameters():
            normal_param = torch.nn.Parameter(torch.ones(1), requires_grad=False)
            lora_param_a = torch.nn.Parameter(torch.ones(1), requires_grad=True)
            lora_param_b = torch.nn.Parameter(torch.ones(1), requires_grad=True)
            return [
                ('layer.0.weight', normal_param),
                ('layer.0.lora.A.weight', lora_param_a),
                ('layer.0.lora.B.weight', lora_param_b)
            ]
        
        mock_model.named_parameters = mock_named_parameters
        
        # 檢查 requires_grad 屬性以確認哪些參數會被更新
        param_status = []
        for name, param in mock_model.named_parameters():
            param_status.append((name, param.requires_grad))
        
        # 檢查是否只有 LoRA 參數設置為 requires_grad=True
        for name, requires_grad in param_status:
            if 'lora' in name:
                assert requires_grad, f"LoRA 參數 {name} 應設置為 requires_grad=True"
            else:
                assert not requires_grad, f"非 LoRA 參數 {name} 應設置為 requires_grad=False"
        
        # 記錄參數更新信息
        with open(os.path.join(self.output_dir, "lora_parameter_updates.txt"), "w", encoding="utf-8") as f:
            f.write("LoRA 參數更新分析\n\n")
            f.write("參數requires_grad設置:\n")
            for name, requires_grad in param_status:
                f.write(f"- {name}: {requires_grad}\n")
            
            f.write("\nLoRA架構說明:\n")
            f.write("1. LoRA是一種參數有效的微調方法，僅更新低秩適應矩陣而非所有模型權重\n")
            f.write("2. LoRA參數通常包含A和B兩個權重矩陣，構成Y = X + BA(X)的形式\n")
            f.write("3. 在正向傳播中，原始權重和LoRA權重的輸出相加形成最終結果\n")
            f.write("4. 通過只更新LoRA參數，可以將可訓練參數減少到原模型的約1%\n") 